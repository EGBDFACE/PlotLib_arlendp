(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('d3'), require('pixi.js')) :
        typeof define === 'function' && define.amd ? define('d3-svg2webgl', ['exports', 'd3', 'pixi.js'], factory) :
        (factory((global.d3 = global.d3 || {}), global.d3, global.PIXI));
}(this, (function (exports, d3, PIXI) {
    'use strict';
    ...
    function color2hex(color) {
        return rgb2hex(d3.color(color).rgb().toString());
    ...
    var transform = function (node) {
        ...
        if (!node.attrs) {
            node.rootNode.stage.addChild(container);
        ...
        container.setTransform(transX, transY, scaleX, scaleY, rotation);
        node.rootNode.stage.addChild(container);
        node.rootNode.activeContainer = container;
        return container;
    };
    ...
    function draw(node, point) {
        var children = node.countNodes,
            ...
            factor = node.factor,
            container = node.rootNode.activeContainer || {},
            attrs = node.attrs;
        ...
        if (drawer === false) return;
        else if (node.tagName !== 'g' && node.attrs) {
            ...
            //
            // Stroke
            stroke = {color: color2hex((node.getValue('stroke') === 'none' ? undefined : node.getValue('stroke')) || '#000'), alpha: node.getValue('stroke-opacity')};
            ...
            //
            // Fill
            fill = { color: color2hex((node.getValue('fill') === 'none' ? undefined : node.getValue('fill')) || '#000'), alpha: node.getValue('fill-opacity')};
            // if (fill) ctx.fillStyle = fill;
            // fill === 'none' || !fill ? false : fill;
            //
            if (drawer) graphic = node.tagName === 'text' ? drawer(node, graphic, stroke, fill, point) : graphic;
        ...
        if (path) {
            ...
            for (var i = 0; i < pathArr.length; i++) {
                ...
                if (index !== -1) {
                    ...
                    var data = pathArr[i].substring(index + 1);
                    if (sign === 'M') {
    ...
    var text = function (node) {
        ...
        text.style.fontFamily = node.getValue('font-family');
