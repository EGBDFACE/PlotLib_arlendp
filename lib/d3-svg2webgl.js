(function (global, factory) {
    ...
    d3.selection.prototype.render = render;
    ...
    function asCanvas(reset) {
        ...
        if (isCanvas(node) && reset) {
            ...
            node.renderer.view.style.width = canvasSize.w + 'px';
            node.renderer.view.style.height = canvasSize.h + 'px';
    ...
    var circle = function (node, graphic, stroke, fill, point) {
        var attrs = node.attrs,
            ctx = node.context,
            f = node.factor;
        graphic.lineColor = stroke || 0;
        graphic.beginFill(fill || 0);
    ...
    var rect = function (node, graphic, stroke, fill, point) {
        ...
        graphic.lineColor = stroke || 0;
        graphic.beginFill(fill || 0);
        graphic.drawRect(0, 0, widht, height);
        graphic.endFill();
        if (point && graphic.containsPoint(point.x, point.y)) point.nodes.push(node);
    };
